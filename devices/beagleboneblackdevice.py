...
class BeagleBoneBlackDevice(Device):
        ...
        for _ in range(self._SERVICE_MODE_RETRY_ATTEMPTS):
            try:
                ...
                if config.BEAGLEBONE_HARNESS:
                    ...
                    serial_write(stream, "setenv bootargs 'console=ttyO0," +
                                         "115200n8, root=/dev/sda1 rootwait " +
                                         "rootfstype=ext4 rw'", 1)
                    serial_write(stream, "usb start", 5)
                    serial_write(stream, "ext4load usb 0:1 0x81000000 "
                                         "/boot/vmlinuz-4.4.9-ti-r25", 10)
                    serial_write(stream, "ext4load usb 0:1 0x80000000 "
                            "/boot/dtbs/4.4.9-ti-r25/am335x-boneblack.dtb", 5)
    ...
    def _remove_temp_dir(self):
        ...
        if config.BEAGLEBONE_HARNESS:
            ...
            shutil.rmtree(os.path.join(self.nfs_path,
                                       self.working_directory[1:]))
